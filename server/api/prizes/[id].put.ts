...
export default defineEventHandler(async (event) => {
  try {
    ...
    const updateQuery = `UPDATE prize SET ${fields.map(field => `${field} = ?`).join(', ')} WHERE id = ?`;
    ...
    throw createError({
      statusCode: 500,
      statusMessage: 'Erreur lors de la mise à jour du prix',
      data: error
    });
  }
});
